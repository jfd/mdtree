...
import GroffRe from "../res/groffre.mjs";

import * as MDNode from "./MDNode.mjs";

export {render};

const groffEscapeRe = new Regexp(GroffRe, "g");
...
function renderHeading(buffer, node) {
    ...
    write(buffer, `${type} ${normalize(node.body)}`);
}
...
function groffEscape(value) {
    return value.replace(groffEscapeRe, a => {
        switch (a) {
